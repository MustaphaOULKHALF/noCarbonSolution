...
BEGIN  
   SELECT COALESCE(SUM(POINTS),0) as Balance, COALESCE(SUM(ReducedCarb),0) as TotalImpact FROM nocarbon.historic 
...
BEGIN  
   SELECT H.CustomerId, c.Name as CategoryName, A.Name as ActionName,COALESCE(H.Points,0) AS Points,COALESCE(H.ReducedCarb,0) AS ReducedCarb, H.OperationDate, H.OperationTime FROM nocarbon.historic H
	INNER JOIN nocarbon.category c ON H.CategoryId = c.Id
	INNER JOIN nocarbon.actions A ON H.ActionId = A.Id
     -- add where condition if required
