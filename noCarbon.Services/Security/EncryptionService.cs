...
public class EncryptionService : IEncryptionService
{
    ...
    private byte[] EncryptTextToMemory(string data, byte[] key, byte[] iv)
    {
        ...
        using (var cs = new CryptoStream(ms, tripleDESCryptoServiceProvider.CreateEncryptor(key, iv), CryptoStreamMode.Write))
        {
    ...
    private string DecryptTextFromMemory(byte[] data, byte[] key, byte[] iv)
    {
        ...
        using var cs = new CryptoStream(ms, tripleDESCryptoServiceProvider.CreateDecryptor(key, iv), CryptoStreamMode.Read);
    ...
    public virtual string EncryptText(string plainText, string encryptionPrivateKey = "")
    {
        ...
        if (string.IsNullOrEmpty(encryptionPrivateKey))
            encryptionPrivateKey = _appSettings.SecurityConfig.EncryptionKey;
